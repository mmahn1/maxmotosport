document.addEventListener("DOMContentLoaded", function () {
    ...
    loadMotorcycleWear(jsonFile);

    function loadMotorcycleWear(jsonPath) {
        fetch(jsonPath)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Failed to load motorcycle wear: ${response.status} ${response.statusText}`);
                }
            ...
            .then(accessories => {
                ...
                const filteredAccessories = accessories.filter(accessory => 
                    accessory.category && accessory.category.toLowerCase() === "bike wear"
                );
                ...
                if (filteredAccessories.length > 0) {
                    ...
                    container.innerHTML = "<p>No motorcycle wear available.</p>";
                }
            })
            .catch(error => {
                console.error("Error loading motorcycle wear:", error);
                container.innerHTML = `<p>Error loading motorcycle wear.</p>`;
            });
    }
});
